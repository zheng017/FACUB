...
void global_gr(int n, double *par, double *gr, void *ex) {
  ...
  vec grad_B_vec = trans(sum(Common, 0));
...
double sigma_fn(int n, double *par, void *ex) {
  ...
  rowvec sigma_i = exp(trans(log_sigma));
...
List FAVA_cpp(SEXP R_in, SEXP V_in, double m, int n_factors, 
  ...
  mat pai_in = ensure_mat(initials["pai"]); 
  ...
  if (pai_in.n_elem == p) {
    ...
    else new_pai = repmat(pai_in, n, 1);
  } else if (pai_in.n_rows == n && pai_in.n_cols == p) {
    new_pai = pai_in;
  } else {
    stop("Dimension mismatch: 'pai' must be length p or size n x p");
  }
  ...
  while ( (diff > eps * (std::abs(cur_VLB) + eps)) && (iter <= maxit) ) {
    ...
    while (!inner_converged && p_iter < 10) {
      ...
      for(int i=0; i<n; ++i) {
        ...
        RowData s_data = {X.row(i), new_mu.row(i), new_alpha.row(i), R_mat.row(i), 
                          new_Lambda, new_beta, B_row, new_sigma.row(i), m, p}; 
  ...
  mat B_mat = repmat(trans(new_B), n, 1);
  mat ll = B_mat + X * new_beta.t() + new_mu * new_Lambda.t();
  mat e_mat = ll + 0.5 * (new_sigma * square(new_Lambda).t());
