...
hic = function(R, V=NULL, m, r) {
  n = nrow(R)
  p = ncol(R)
  aic = bic = hic1 = hic2 = hic3 = hic4 = hic5 = numeric(r)
  sn = log(n)/2
  for (w in 1:r) {
    re = FAVA_new(R, V, m, w)
    aic[w] = -2*re$lob + 2*(w*p + p*q + n*w)     # aic
    bic[w] = -2*re$lob + log(n*p)*(w*p + 2*p + n*p + 2*n*w - w^2)  # bic in jasa(2022)
    hic1[w] = -2*re$lob + log(n)*(w*p - w^2) + 2*w*n  # hic1 in JCGS(2021)
    hic2[w] = -re$lob + sn*(w*p + p*q + re$EE)    # hic2 with sn=log(n)/2 in JASA(2023)
    hic3[w] = -re$lob + (w*p + p*q + re$EE)     # hic3 with sn=1 in JASA(2023)
    hic4[w] = -re$EN2                          # hic4 自己定义的
    hic5[w] = -re$EN2 + w*p + p*q              # hic5自己定义的
  }
  
  return(list(aic = aic, bic = bic, hic1=hic1, hic2=hic2, hic3=hic3, hic4=hic4, hic5=hic5))
}
...
hic_new = function(R, V=NULL, m, r) {
  ...
  #aic2=bic2=hic11=hic22=hic33=numeric(r)
  sn = log(n)/2
  for (w in 1:r) {
...
hic_KLAnnealing_new = function(R, V=NULL, m, r) {
  ...
  for (w in 1:r) {
    ...
    aic[w] = -2*re$EE + 2*(w*p + n*w-w^2)     # aic
    bic[w] = -2*re$EE + log(n*p)*(w*p + 2*n*w - w^2)  # bic in jasa(2022)
    hic1[w] = -2*re$EE + log(n)*(w*p-w^2) + 2*(w*n)  # hic1 in JCGS(2021)
    hic2[w] = -re$EE + sn*(w*p -w^2 +w*n- re$EN2)    # hic2 with sn=log(n)/2 in JASA(2023)
    hic3[w] = -re$EE + (w*p -w^2 + w*n- re$EN2)     # hic3 with sn=1 in JASA(2023)
  }
...
hic_old = function(R, V=NULL, m, r) {
  n = nrow(R)
  p = ncol(R)
  aic = bic = hic1 = hic2 = hic3 = EE = numeric(r)
  #aic2=bic2=hic11=hic22=hic33=numeric(r)
  sn = log(n)/2
  for (w in 1:r) {
    initials = generate_initials(R, V, w, type = "3", 0, 0, 0.1, 0.5)
    re = FAVA_new(R, V, m, w, initials)
    aic[w] = -2*re$EE + 2*(w*p + n*w-w^2)     # aic
    bic[w] = -2*re$EE + log(n*p)*(w*p + 2*n*w - w^2)  # bic in jasa(2022)
    hic1[w] = -2*re$EE + log(n)*(w*p-w^2) + 2*(w*n)  # hic1 in JCGS(2021)
    hic2[w] = -re$EE + sn*(w*p -w^2 +w*n- re$EN2)    # hic2 with sn=log(n)/2 in JASA(2023)
  ...
  return(list(aic = aic,bic = bic,hic1=hic1, hic2=hic2, hic3=hic3, EE = EE))
}




FAVA_new3 = function(R, V=NULL, m, n.factors, initials, maxit=200, trace=FALSE) {
  #newR = check_R(R, m)
  #R = newR$R
  #re_order = newR$reorder
  n = nrow(R)
  ...
  # --- MODIFICATION START: Define Annealing Steps ---
  anneal_steps = 50 
  # --------------------------------------------------
  
  while ( (diff > eps*(abs(cur.VLB)+eps)) && iter <= max.iter ) {
    
    # --- MODIFICATION START: Calculate beta_kl ---
    if (iter <= anneal_steps) {
      beta_kl = iter / anneal_steps
    } else {
      beta_kl = 1
    }
    # --------------------------------------------------
    
    cat("=======================\nIteration: ", iter, " (KL Weight: ", beta_kl, ")\n")
    cat(" [Outer ", iter, "] j=1 Lambda START: ", new.Lambda[1, ], "\n")
    cat(" [Outer ", iter, "]  pai START: ", new.pai, "\n")
    cat(" [Outer ", iter, "]  B START: ", new.B, "\n")
    ...
    VLB = function(model.coefs, va.mu, va.sigma, pai, alpha) {
      ...
      # Note: For VLB monitoring, we usually want to see the "Real" VLB (beta=1), 
      # but checking the "Annealed" VLB helps tracking the optimizer's actual objective.
      # Here I apply beta_kl so it matches the gradients.
      fun1 = -0.5 * beta_kl * ( sum(new.sigma) + sum(new.mu^2) - sum(log(new.sigma)) )
    ...
    while (!all(delta.alpha < delta.alpha.required) & (p.iter < p.max.iter)) {
      ...
      pai.mat = matrix(new.pai, n, p, byrow = TRUE)
      new.alpha = plogis( log(pai.mat / (1-pai.mat)) + log(choose(m-1, R-1)) + (m-R)*ll - (m-1)*log(1+exp(e.mat)) + log(m) )
      cat(" [Inner ", p.iter, "] alpha START: ", new.alpha[1, ], "\n")
      ...
      va.sigma_f = function(model.coefs, va.mu, va.sigma, pai, alpha) {
        ...
        # --- MODIFICATION START: beta_kl added ---
        fun1 = -0.5 * beta_kl * ( sum(new.sigma) - sum(log(new.sigma)) )
        # -----------------------------------------
      ...
      va.sigma_gr_f = function(model.coefs, va.mu, va.sigma, pai, alpha) {
        ...
        if (n.factors == 1) {
          for (i in 1:n) {
            # --- MODIFICATION START: beta_kl added ---
            grad.sigma[i, ] = -0.5 * beta_kl * (1 - new.sigma[i, ]^(-1)) + apply(mu.mat[i, ]*beta2, 2, sum)
          }
        } else {
          for (i in 1:n) {
            # --- MODIFICATION START: beta_kl added ---
            grad.sigma[i, ] = diag(-0.5 * beta_kl * (diag(rep(1, n.factors)) - diag(1/new.sigma[i, ], n.factors)) + diag(apply(mu.mat[i, ]*beta2, 2, sum)))
            # -----------------------------------------
          }
        }
        return(c(grad.sigma))
      ...
      va.sigma_fi = function(model.coefs, va.mu, va.sigma, 
        ...
        # --- MODIFICATION START: beta_kl added ---
        y1 = -0.5 * beta_kl * (sum(new.sigma) - sum(log(new.sigma)))
        # -----------------------------------------
      ...
      va.sigma_gr_fi = function(model.coefs, va.mu, va.sigma, 
        ...
        # --- MODIFICATION START: beta_kl added ---
        grad.sigma = diag(-0.5 * beta_kl * (diag(rep(1, n.factors), n.factors) - diag(new.sigma^(-1), n.factors)) + 
                            diag(colSums(mu.vec * beta2), n.factors))
        # -----------------------------------------
      ...
      for (i in 1:n) {
        if (i == 1) {
          cat(" [Inner ", p.iter, "] i=0 SIGMA START: ", new.sigma[i, ], "\n")
        }
        ...
        if ("try-error" %in% class(qq)) {
          ...
          if (iter > 1 && s.cur.logfunc > qq$value) {
            ...
            if (i == 1) cat("Variational parameters sigma updated\n")
            new.sigma[i, ] = qq$par
            if (qq$convergence != 0) {
              if (i == 1) cat("Optimization of sigma did not converge!\n")
              if (trace) {
                cat("Optimization of sigma did not converge on iteration step ",
                    iter, "\n")
              }
      ...
      mu_grad_f = function(model.coefs, va.mu, va.sigma, pai, alpha) {
        ...
        for (w in 1:n.factors) {
          # --- MODIFICATION START: beta_kl added ---
          grad.m = c(grad.m, rowSums(sweep(sum1, 2, new.Lambda[, w], "*")) - beta_kl * new.mu[, w])
          # -----------------------------------------
        }
        return(c(grad.m))
      }
      ...
      mu_grad_fi = function(model.coefs, va.mu, va.sigma, pai, alpha, ii) {
        ...
        # --- MODIFICATION START: beta_kl added ---
        return(-beta_kl * new.mu + as.vector(crossprod(sum1, new.Lambda)))
        # -----------------------------------------
      }
      for (i in 1:n) {
        if (i == 1) {
          cat(" [Inner ", p.iter, "] i=0 MU START: ", new.mu[i, ], "\n")
        }
        qq = try(optim(mu[i, ], method = "BFGS", fn = mu_fi,
                       gr = mu_grad_fi,
                       model.coefs = c(new.Lambda, new.B, new.beta),
        ...
        if ("try-error" %in% class(qq)) {
          new.mu[i, ] = mu[i, ]
          if (i == 1) print("Optimization of mu did not improve on iteration step")
        } else {
          if (iter > 1 && m.cur.logfunc > qq$value) {
            if (i == 1) print("Optimization of mu did not improve on iteration step")
            if (trace) {
            ...
            if (i == 1) cat("Variational parameters mu updated\n")
            new.mu[i, ] = qq$par
            if (qq$convergence != 0) {
              if (i == 1) print("Optimization of mu did not improve on iteration step")
              if (trace) {
...
FAVA_new4 = function(R, V=NULL, m, n.factors, initials, maxit=200, trace=FALSE) {
  #newR = check_R(R, m)
  #R = newR$R
  #re_order = newR$reorder
  n = nrow(R)
  ...
  ### MODIFICATION 1 START: Define Annealing Steps ###
  anneal_steps = 50 
  ### ------------------------------------------- ###
  
  while ( (diff > eps*(abs(cur.VLB)+eps)) && iter <= max.iter ) {
    
    ### MODIFICATION 1 START: Calculate beta_kl ###
    if (iter <= anneal_steps) {
      beta_kl = iter / anneal_steps
    } else {
      beta_kl = 1
    ...
    VLB = function(model.coefs, va.mu, va.sigma, pai, alpha) {
      ...
      ### MODIFICATION 1: Apply beta_kl to prior term ###
      fun1 = -0.5 * beta_kl * ( sum(new.sigma) + sum(new.mu^2) - sum(log(new.sigma)) )
    ...
    # update pai
    ### MODIFICATION 2 START: MAP Estimation (Smoothing) ###
    # Old Code: new.pai = round(apply(new.alpha, 2, mean), 6)
    a_prior = 2
    b_prior = 2
    new.pai = round( (colSums(new.alpha) + a_prior - 1) / (n + a_prior + b_prior - 2), 6)
    ### ------------------------------------------------ ###
...
FAVA_new5 = function(R, V=NULL, m, n.factors, initials, maxit=200, trace=FALSE) {
  #newR = check_R(R, m)
  #R = newR$R
  #re_order = newR$reorder
  n = nrow(R)
  ...
  ### MODIFICATION 1: Define Annealing Steps ###
  anneal_steps = 50 
  ### -------------------------------------- ###
  ...
  while ( (diff > eps*(abs(cur.VLB)+eps)) && iter <= max.iter ) {
    ...
    # update pai
    ### MODIFICATION 2: MAP Estimation (Smoothing) ###
    prior_strength = 0.05 * n 
    
    # Center the prior around 0.5 (or your expected mean)
    # If we assume pai is around 0.5, we set success/failure priors equally.
    a_prior = prior_strength / 2
    b_prior = prior_strength / 2
    
    # The MAP update
    new.pai = round( (colSums(new.alpha) + a_prior - 1) / (n + a_prior + b_prior - 2), 6)
    
    # OPTIONAL: Hard cap to prevent numerical locking at 1.0
    # This forces pai to never exceed 0.99, ensuring the Uniform component always exists mathematically.
    new.pai = pmin(new.pai, 0.99)
    ### ------------------------------------------ ###
    ...
    ### MODIFICATION 3: Robust Inner Loop Stopping ###
    p.iter = 1
    p.max.iter = 10 
    inner_converged = FALSE
    delta.alpha.required = 1e-3
    
    while (!inner_converged & (p.iter <= p.max.iter)) {
      
      # Save old values to check for convergence
      old.alpha = new.alpha
      old.mu = new.mu
      old.sigma = new.sigma
      ...
      for (i in 1:n) {
        ...
        if ("try-error" %in% class(qq)) {
          ...
          if (iter > 1 && s.cur.logfunc > qq$value) {
            ...
            if (i == 1 && trace) cat("Variational parameters sigma updated\n")
            new.sigma[i, ] = qq$par
            if (qq$convergence != 0) {
              if (i == 1 && trace) cat("Optimization of sigma did not converge!\n")
              if (trace) {
                cat("Optimization of sigma did not converge on iteration step ",
                    iter, "\n")
              }
      ...
      for (i in 1:n) {
        ...
        if ("try-error" %in% class(qq)) {
          new.mu[i, ] = mu[i, ]
          if (i == 1 && trace) print("Optimization of mu did not improve on iteration step")
        } else {
          if (iter > 1 && m.cur.logfunc > qq$value) {
            if (i == 1 && trace) print("Optimization of mu did not improve on iteration step")
            if (trace) {
            ...
            if (i == 1 && trace) cat("Variational parameters mu updated\n")
            new.mu[i, ] = qq$par
            if (qq$convergence != 0) {
              if (i == 1 && trace) print("Optimization of mu did not improve on iteration step")
              if (trace) {
      ...
      ### MODIFICATION 3: Check for Early Stopping ###
      d.alpha = max(abs(new.alpha - old.alpha))
      d.mu    = max(abs(new.mu - old.mu))
      d.sigma = max(abs(new.sigma - old.sigma))
      
      if (d.alpha < delta.alpha.required && d.mu < 1e-3 && d.sigma < 1e-3) {
